...
Array.prototype.oddOneOut = function() {
...
let input = `mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
console.log(problem1(input_arr));
console.log(problem2(input_arr));

function problem1(input){
	let set_left_side = new Set();
	let set_right_side = new Set();

	let regex = /^([a-z]+) (\([0-9]+\))(?: -> ((?:[a-z]+(, )?)+))?/;
	for (str of input){
		let match = str.match(regex);
		let left_hand_side = match[1];
		let right_hand_side = match[3];
		set_left_side.add(left_hand_side);
		if (right_hand_side){
			let right_side_array = right_hand_side.split(', ');
			right_side_array.forEach( x => set_right_side.add(x));
		}
	}

	return set_left_side.difference(set_right_side);

}

function problem2(input){
	let queue = [];
	problem1(input).forEach(x => queue.push(x));
	let regex = /^([a-z]+) \(([0-9]+)\)(?: -> ((?:[a-z]+(, )?)+))?/;
	let weights = new Map();
	let towers = new Map();
	let tower_weights = new Map();
	...
	for (str of input){
		...
		weights.set(left_hand_side, weight);
		if (right_hand_side){
			let right_side_array = right_hand_side.split(', ');
			towers.set(left_hand_side, right_side_array);
		}
	}
	while (queue.length > 0){
		...
		if (!visited.has(current)){
			queue.push(current);
			if (towers.has(current)){
				towers.get(current).forEach( x => queue.push(x));
			}
			visited.add(current);
		}
		else{
			let isBalanced = towerIsBalanced(towers.get(current), current, towers, weights, tower_weights);
			if (isBalanced !== -1){
				return isBalanced;
			}
		}
...
function getTowerWeight(tower, key, towers, weights, tower_weights){
	let weight = weights.get(key);
	if (!tower){
		return weight;
	}
	if (tower_weights.has(key)){
		return tower_weights.get(key);
	}
	tower.forEach(program => {
		weight += getTowerWeight(towers.get(program), program, towers, weights, tower_weights);
	});
	tower_weights.set(key, weight);
	return weight;
}

function towerIsBalanced(tower, key, towers, weights, tower_weights){
	if (!tower){
		return -1;
	}
	let balancing_weights = [];
	for (let balancing_tower of tower){
		balancing_weights.push(getTowerWeight(towers.get(balancing_tower), balancing_tower, towers, weights, tower_weights));
	}
	let i = balancing_weights.oddOneOut();
	if (i !== -1){
		let diff = balancing_weights[i] - balancing_weights[(i+1)%balancing_weights.length];
		return weights.get(tower[i]) - diff;
	}
	return -1;
