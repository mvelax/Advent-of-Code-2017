...
console.log(followInstructions(input_arr));

function followInstructions(input){
	let registers = new Map();
	const format = /^([a-z]+) (inc|dec) (-?[0-9]+) if ([a-z]+) ((?:[<>]=?)|[!=]=) (-?[0-9]+)$/;
	let max;
	for (let instruction of input_arr){
		const match = instruction.match(format);
		const register = match[1];
		const inc_dec = match[2];
		const amount = Number(match[3]);
		const condition_register = match[4];
		const operator = match[5];
		const operand = Number(match[6]);
		
		if (conditionIsMet(condition_register, operator, operand, registers)){
			execute(register, inc_dec, amount, registers);
			if (max === undefined){
				max = Array.from(registers.values()).reduce( (current_max, next_val) => Math.max(current_max, next_val));
			}
			else{
				max = Math.max(max, Array.from(registers.values()).reduce( (current_max, next_val) => Math.max(current_max, next_val)));
			}
		}
	}
...
function conditionIsMet(condition_register, operator, operand, registers){
	if (!registers.has(condition_register)){
		registers.set(condition_register, 0);
	}
	const left_side = registers.get(condition_register);
	return eval(`${left_side} ${operator} ${operand}`);
}

function execute(register, inc_dec, amount, registers){
	if (!registers.has(register)){
		registers.set(register, 0);
	}
	const multiplier = inc_dec === "inc" ? 1 : -1;
	registers.set(register, registers.get(register) + (multiplier*amount));
}